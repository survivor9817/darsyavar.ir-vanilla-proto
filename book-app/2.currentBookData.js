...
const currentBookData = {
  ...
  // ---- Getters ----
  get selectedBookName() { return this._selectedBookName; },
  get fehrestOfSelectedBook() { return this._fehrestOfSelectedBook; },
  get contentsOfSelectedBook() { return this._contentsOfSelectedBook; },
  get fehrestRefTitlePagesArray() { return this._fehrestRefTitlePagesArray; },
  get observingPageNumber() { return this._observingPageNumber; },
  ...
  updateSelectedBook(bookName) {
    ...
    this._fehrestRefTitlePagesArray = this._fehrestOfSelectedBook.map(item => item.refPage);
  ...
  updateObservingPageNumber(newPageNumber) {
    ...
    pageInputNumberElement.value = newPageNumber;
  ...
  goToPage(number) {
    ...
    const targetPage = document.getElementById(`page${this._observingPageNumber}`);
  ...
  goToNextPage() {
    ...
    const newPage = Math.min(Number(maxPage), Number(this._observingPageNumber) + 1);
    this.goToPage(newPage);
  }
};
